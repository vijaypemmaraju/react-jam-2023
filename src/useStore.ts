...
const useStore = create<Store>((set, get) => ({
  ...
  updatePlayer: (delta: number) => {
    const { player: oldPlayer, setPlayer, WEIGHTS, audioDataArray } = get();
  ...
  updateBirds: (delta: number) => {
    const { player, birds, rivals, setBirds, WEIGHTS, audioDataArray } = get();
    ...
    for (let i = 0; i < birds.length; i++) {
      ...
      const playerVelocityThresholdRatio = isActuallyCloseToPlayer
        ? Math.pow(player.acceleration / (WEIGHTS.HIGH_SPEED_THRESHOLD / 2), 4)
        : 0;
  ...
  updateRivals: (delta: number) => {
    ...
    for (let i = 0; i < oldRivals.length; i++) {
      const rival = { ...oldRivals[i] };
